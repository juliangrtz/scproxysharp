...
{
    private static readonly Dictionary<int, string> ClashRoyalePackets = new()
    {
        { 10100, "ClientHello" },
        { 10101, "Login" },
    ...
    public static string GetPacketType(int messageID)
    {
        ...
        {
            case SupercellGame.CLASH_ROYALE:
                if (ClashRoyalePackets.ContainsKey(messageID))
                    ClashRoyalePackets.TryGetValue(messageID, out ret);
                break;
